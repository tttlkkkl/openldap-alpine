...
rd=root
if [ ! -d $CERT_DIR/$rd ]; then
  mkdir -p $CERT_DIR/$rd
fi
if [ ! -f "$CERT_DIR/$rd/$ROOT_NAME.crt" ]
then
  ...
  openssl genrsa \
    -out "$CERT_DIR/$rd/$ROOT_NAME.key" \
    "$RSA_KEY_NUMBITS"

  echo " ---> Generate Root CA certificate request"
  openssl req \
    -new \
    -key "$CERT_DIR/$rd/$ROOT_NAME.key" \
    -out "$CERT_DIR/$rd/$ROOT_NAME.csr" \
    -subj "$ROOT_SUBJ"
  ...
  openssl req \
    -x509 \
    -key "$CERT_DIR/$rd/$ROOT_NAME.key" \
    -in "$CERT_DIR/$rd/$ROOT_NAME.csr" \
    -out "$CERT_DIR/$rd/$ROOT_NAME.crt" \
    -days "$DAYS"
else
  echo "ENTRYPOINT: $ROOT_NAME.crt already exists"
fi

if [ ! -f "$CERT_DIR/$rd/$ISSUER_NAME.crt" ]
then
  ...
  openssl genrsa \
    -out "$CERT_DIR/$rd/$ISSUER_NAME.key" \
    "$RSA_KEY_NUMBITS"

  echo " ---> Generate Issuer certificate request"
  openssl req \
    -new \
    -key "$CERT_DIR/$rd/$ISSUER_NAME.key" \
    -out "$CERT_DIR/$rd/$ISSUER_NAME.csr" \
    -subj "$ISSUER_SUBJ"

  echo " ---> Generate Issuer certificate"
  openssl x509 \
    -req \
    -in "$CERT_DIR/$rd/$ISSUER_NAME.csr" \
    -CA "$CERT_DIR/$rd/$ROOT_NAME.crt" \
    -CAkey "$CERT_DIR/$rd/$ROOT_NAME.key" \
    -out "$CERT_DIR/$rd/$ISSUER_NAME.crt" \
    -CAcreateserial \
    -extfile issuer.ext \
    -days "$DAYS"
else
...
then
  ...
  echo -e "\nDNS.1 = $PUBLIC_CN" >> public.ext

  echo " ---> Generate public certificate signed by $ISSUER_CN"
  openssl x509 \
    -req \
    -in "$CERT_DIR/$PUBLIC_NAME.csr" \
    -CA "$CERT_DIR/$rd/$ISSUER_NAME.crt" \
    -CAkey "$CERT_DIR/$rd/$ISSUER_NAME.key" \
...
then
  # 将根证书和发布者证书合并为 ca.pem
  echo " ---> Generate a combined root and issuer ca.pem"
  cat "$CERT_DIR/$rd/$ISSUER_NAME.crt" "$CERT_DIR/$rd/$ROOT_NAME.crt" > "$CERT_DIR/ca.pem"
else
